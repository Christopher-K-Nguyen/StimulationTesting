function [scale_new,pos_new] = setFineScalePos(File,scopeChannel,YUNits_min,YUNits_max,YUNits_offset,fineScale_min)
%% Function
if ~contains2(scopeChannel,'MATH')
    ...
    scale_new = str2double(scale_char) / 2;
    if scale_new < fineScale_min
        scale_new = fineScale_min;
    end
    ...
    fprintf('(%.2e V)...',scale_new);

    %% Adjust position
    fprintf('positioning ');
    pos_scale = mean([YUNits_min YUNits_max]);
    if ~isempty(YUNits_offset)
        pos = mean([pos_scale YUNits_offset]);
    else
        pos = pos_scale;
    end
    if pos > 0
