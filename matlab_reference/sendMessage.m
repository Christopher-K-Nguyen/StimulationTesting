...
if numOfVar > 0
    if ~isempty(msg)
        msg_1 = sprintf('%s at %.2f %s.',msg,endTime,unit);
    else
        msg_1 = sprintf('Experiment completed at %.2f %s.',endTime,unit);
    end
else
    msg_1 = sprintf('Experiment "%s" completed on %s.',filename,getDateTIme());
end
msg_2 = 'The file(s) from the experiment is attached to this message.';
