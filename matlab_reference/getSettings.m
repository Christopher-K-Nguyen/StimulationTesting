function File = getSettings(File,varargin)
%% Variables
...
setOscilloscopeStatus(File,'open');
tag = '';
isChannelOn = true;
...
if isTPS || contains2(source,'CH2')
    if contains2(source,'CH2')
        fprintf(oscilloscope,'*CLS');
    end
    source_check = '';
    while ~contains2(source,source_check)
        ...
        if ~contains2(source,source_check)
            fprintf(oscilloscope,'*CLS');
        end
    end
else
