function [File,fig] = getPlot2_Tek(File)
%% Constants
FIRST_COLOR = '#0072BD';
SECOND_COLOR = '#D95319';
THIRD_COLOR = '#EDB120';
...
for deviceNum = 1:numOfDevices
    ...
    Monitor = File.Data(prevCaptureNum).Monitor(deviceNum);
    ...
    if ~hasMonitor
        ...
        cursorTime_arr = [];
        File.Data(captureNum).Monitor(deviceNum).Time = cursorTime_arr;
    end
end
...
for deviceNum = 1:numOfDevices
    ...
    if deviceNum ~= prevDeviceNum
        prevNumOfChannels = numOfChannels;
    else
        prevNumOfChannels = 0;
    end
    for channel_idx = 1:numOfChannels
        lineNum = channel_idx + prevNumOfChannels;
        ...
        if contains2(channel,'curr')
            ...
            lines(lineNum) = plot(time,currentDensity, ...
                'Color',color, ...
                'LineStyle','-', ...
                'LineWidth',1, ...
        ...
        if contains2(channel,monitorChannel_cell)
            ...
            File.Data(captureNum).Monitor.Cursor.Value = cursorValue_arr;
            scatter(cursorTime_arr,cursorValue_arr,'filled','x', ...
            ...
            text(cursorTime_arr,cursorValue_arr,textLabel, ...
