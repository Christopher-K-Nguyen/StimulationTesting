function [isDone,isQuit] = runPulsing3(File)
close all;
try
...
plexonOmnetics = File.Parameters.Channels.Plexon;
...
for channelNum = 1:NUM_OF_CHANNELS    % consecutively stimulate channels
    ...
    if contains(subjectSelect,'C')
        channelStim = plexonOmnetics(channelNum);
    else
    ...
    isQuit = setStimRate(1,channelStim,stimRate);
    if isQuit
        return;
    end

    % Set number of repetitions for all stimulators
    [isQuit] = setNumOfPulses(1,channelStim,numOfPulses);
    if isQuit == true
        return;
    end
    ...
    isQuit = setStimParam(1,channelStim,pattern);
    if isQuit
        return;
    end

    % Load parameters to channel
    [isQuit] = loadChannel(1,channelStim);
    if isQuit == true
        return;
    end
...
isQuit = setMonitorChannel(1,channel_max);
if isQuit
    return;
end
