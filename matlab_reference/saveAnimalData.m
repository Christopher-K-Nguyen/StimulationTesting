function [file_mat,file_xlsx] = saveAnimalData(...
    filepath,...
    filename,...
    data)
%% Constants
...
file_mat = append(filename,'.mat');       	% filename for .mat file
...
file_xlsx = append(filename,'.xlsx');           % filename for .xlsx file
file_xlsx_save = fullfile(filepath,file_xlsx); 	% save path for .xlsx file
...
for channel_idx = 1:channel_idx_last    % loop through channel data
    ...
    chargePhase = data(channelNum).ChargePerPhase;      	% charge per phase value
    ...
    %     file_csv_char = sprintf('%s_Ch%02d.csv',filename_char,channelNum);  % filename for .csv file (char)
    ...
    writetable(tableData,file_xlsx_save,'Sheet',channelNum_str);    % save sheet to .xlsx file
    channelList_alloc = [channelList,{channelNum_char}];
    channelList = channelList_alloc;
    chargeInjList_alloc = [chargeInjList,chargeInj];
    chargeInjList = chargeInjList_alloc;
    maxPotentialList_alloc = [maxPotentialList,maxPotential];
    maxPotentialList = maxPotentialList_alloc;
    drivingVoltageList_alloc = [drivingVoltageList,voltageDrive];
    drivingVoltageList = drivingVoltageList_alloc;
    fprintf('%d...',channelNum);
end
sheetName = 'Voltage Measurements';
headings = tableHeading(6:8);
valueTable = [chargeInjList;maxPotentialList;drivingVoltageList];
...
%     'VariableNames',channelList,...
%     'RowNames',headings);
writetable(dataTable,file_xlsx_save,'Sheet',sheetName,'WriteRowNames',true);    % save sheet to .xlsx file
fprintf('%s\n',file_xlsx);	% .xlsx file saved
